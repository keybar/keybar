...
DOKKU_DOMAIN = ENV["DOKKU_DOMAIN"] || "keybar.me"
DOKKU_IP = ENV["DOKKU_IP"] || "10.0.0.2"
PREBUILT_STACK_URL = File.exist?("#{File.dirname(__FILE__)}/stack.tgz") ? 'file:///root/keybar/stack.tgz' : nil

make_cmd = "make install-dokku"
...
Vagrant::configure("2") do |config|
  ...
  config.vm.synced_folder File.dirname(__FILE__), "/root/keybar"
  ...
  config.vm.provision :shell, :inline => "apt-get -qq -y install git > /dev/null && cd /root/keybar && #{make_cmd}"
  config.vm.provision :shell, :inline => "cd /root/keybar && make install-dokku"
end
